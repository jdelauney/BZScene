...
// Updated: 02/2020 for version 1.0.3 - J.Delauney (BeanzMaster) (https://www.developpez.net/forums/u912403/beanzmaster/)
...
//-----------------------------------------------------------------------------
  ...
  pad : Array[0..1] of LongWord;  // padding for later use
...
// of source picture. These presets are used when calling WebPConfigPreset().
  ...
  TWebPAuxStats = record
    coded_size : Integer;
    PSNR: array[0..4] of Single;                   // peak-signal-to-noise ratio for Y/U/V/All
    ...
    lossless_features : Longword;  // bit0:predictor bit1:cross-color transform
  ...
  TWebPMemoryWriter = record
    ...
    size : Integer;      // final size
    max_size : Integer;  // total capacity
    pad : Longword;      // padding for later use
  end;
...
// everything is OK.
  TWebPProgressHook = function(percent : Integer; const picture: PWebPPicture) : Integer; cdecl;
...
var
  ...
  WebPMemoryWrite : function(const data : PByte; data_size : Integer; const picture: PWebPPicture) : Integer;cdecl;
  //-----------------------------------------------------------------------------
  // Main call
  ...
  WebPConfigInitInternal: function(const conf: PWebPConfig; preset: TWebPPreset; quality: single; version: Integer): Integer; cdecl;
...
begin
  // < Reference counting
  if LibWebPRefCount > 0 then dec(LibWebPRefCount);
  if LibWebPRefCount > 0 then exit;
